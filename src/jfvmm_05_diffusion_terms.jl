...
function diffusionTerm1D{T<:Real}(D::FaceValue{T})
    ...
    # ??? De & Dw should be of the right shape already
    AE = reshape(De,Nx)
    AW = reshape(Dw,Nx)
    APx = -(AE+AW)
    # build the sparse matrix based on the numbering system
    # TODO: not sure if the resheps are necessary
...
function diffusionTermCylindrical1D{T<:Real}(D::FaceValue{T})
    Nx = D.domain.dims[1]
    G = [1:Nx+2;]
    DX = D.domain.cellsize.x
    dx = 0.5*(DX[1:end-1]+DX[2:end])
...
# %% ======================== 2D RADIAL DIFFUSION =========================
function diffusionTermRadial2D{T<:Real}(D::FaceValue{T})
    Nr = D.domain.dims[1]
