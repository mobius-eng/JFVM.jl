...
# %% Main functions
# TODO: convectionTerm be careful what is returned...
# TODO: factor the code to remove duplicates

"""
Constructs the matrix for the convection term of the equation
...
Inputs:
- `v`: a field of convective velocity defined on faces

Outputs:
- `Mc`: system matrix corresponding to convection term
"""
convectionTerm(u :: FaceValue) = convectionTerm(u.domain.meshtype, u)

convectionTerm(::Mesh1D, u) = convectionTerm1D(u)
convectionTerm(::Mesh1DPolar, u) = convectionTermCylindrical1D(u)
convectionTerm(::Mesh2D, u) = convectionTerm2D(u)[1]
convectionTerm(::Mesh2DCylindrical, u) = convectionTermCylindrical2D(u)[1]
convectionTerm(::Mesh2DPolar, u) = convectionTermRadial2D(u)[1]
convectionTerm(::Mesh3D, u) = convectionTerm3D(u)[1]
convectionTerm(::Mesh3DCylindrical, u) = convectionTermCylindrical3D(u)[1]
...
using upwind approximation (lower accuracy but more stable)

```julia
Mc = convectioUpwindnTerm(v)
```
...
convectionUpwindTerm(u :: FaceValue) = convectionUpwindTerm(u.domain.meshtype, u)

convectionUpwindTerm(::Mesh1D, u) = convectionUpwindTerm1D(u)
convectionUpwindTerm(::Mesh1DPolar, u) = convectionUpwindTermCylindrical1D(u)
convectionUpwindTerm(::Mesh2D, u) = convectionUpwindTerm2D(u)[1]
convectionUpwindTerm(::Mesh2DCylindrical, u) = convectionUpwindTermCylindrical2D(u)[1]
convectionUpwindTerm(::Mesh2DPolar, u) = convectionUpwindTermRadial2D(u)[1]
convectionUpwindTerm(::Mesh3D, u) = convectionUpwindTerm3D(u)[1]
convectionUpwindTerm(::Mesh3DCylindrical, u) = convectionUpwindTermCylindrical3D(u)[1]


# TODO: check the return value of TVD convection
"""
Constructs the matrix for the convection term of the equation
...
Outputs:
- `Mc`: system matrix corresponding to convection term
- `RHS` vector for convection term
"""
convectionTvdTerm(u :: FaceValue) = convectionTvdTerm(u.domain.meshtype, u)

convectionTvdTerm(::Mesh1D, u) = convectionTvdTerm1D(u)
convectionTvdTerm(::Mesh1DPolar, u) = convectionTvdTermCylindrical1D(u)
convectionTvdTerm(::Mesh2D, u) = convectionTvdTerm2D(u)[1]
convectionTvdTerm(::Mesh2DCylindrical, u) = convectionTvdTermCylindrical2D(u)[1]
convectionTvdTerm(::Mesh2DPolar, u) = convectionTvdTermRadial2D(u)[1]
convectionTvdTerm(::Mesh3D, u) = convectionTvdTerm3D(u)[1]
convectionTvdTerm(::Mesh3DCylindrical, u) = convectionTvdTermCylindrical3D(u)[1]


convectionTvdRHS(u::FaceValue, phi::CellValue, FL::Function) =
    convectionTvdRHS(u.domain.meshtype, u, phi, FL)

convectionTvdRHS(::Mesh1D, u::FaceValue, phi::CellValue, FL::Function) =
    convectionTvdRHS1D(u, phi, FL)

convectionTvdRHS(::Mesh1DPolar, u::FaceValue, phi::CellValue, FL::Function) =
    convectionTvdRHSCylindrical1D(u, phi, FL)

convectionTvdRHS(::Mesh2D, u::FaceValue, phi::CellValue, FL::Function) =
    convectionTvdRHS2D(u, phi, FL)[1]

convectionTvdRHS(::Mesh2DCylindrical, u::FaceValue, phi::CellValue, FL::Function) =
    convectionTvdRHSCylindrical2D(u, phi, FL)[1]

convectionTvdRHS(::Mesh2DPolar, u::FaceValue, phi::CellValue, FL::Function) =
    convectionTvdRHSRadial2D(u, phi, FL)[1]

convectionTvdRHS(::Mesh3D, u::FaceValue, phi::CellValue, FL::Function) =
    convectionTvdRHS3D(u, phi, FL)[1]

convectionTvdRHS(::Mesh3DCylindrical, u::FaceValue, phi::CellValue, FL::Function) =
    convectionTvdRHSCylindrical3D(u, phi, FL)[1]

# =================== 1D Convection Terms =======================
# %% =============== 1D Convection Terms Cartesian Central =================
function convectionTerm1D{T<:Real}(u::FaceValue{T})
    Nx = u.domain.dims[1]
    G = [1:Nx+2;]
    ...
    kx = 3*Nx
    sparse(iix[1:kx], jjx[1:kx], sx[1:kx], Nx+2, Nx+2)
end

# %% =============== 1D Convection Terms Cylindrical Central =================
function convectionTermCylindrical1D{T<:Real}(u::FaceValue{T})
...
function convectionUpwindTerm1D{T<:Real}(u::FaceValue{T})
    # extract data from the mesh structure
    Nx = u.domain.dims[1]
    G = [1:Nx+2;]
    DXp = u.domain.cellsize.x[2:end-1]
    ...
    ue_min = min(u.xvalue[2:Nx+1],zero(T))
    ue_max = max(u.xvalue[2:Nx+1],zero(T))
    uw_min = min(u.xvalue[1:Nx],zero(T))
    uw_max = max(u.xvalue[1:Nx],zero(T))
    # calculate the coefficients for the internal cells
...
function convectionUpwindTermCylindrical1D{T<:Real}(u::FaceValue{T})
    # extract data from the mesh structure
    Nx = u.domain.dims[1]
    G = [1:Nx+2;]
    DXp = u.domain.cellsize.x[2:end-1]
...
function convectionTvdTermCylindrical1D{T<:Real}(u::FaceValue{T}, phi::CellValue{T}, FL::Function)
...
function convectionTerm2D{T<:Real}(u::FaceValue{T})
    # extract data from the mesh structure
    Nx = u.domain.dims[1]
    Ny = u.domain.dims[2]
...
function convectionUpwindTerm2D{T<:Real}(u::FaceValue{T})
# extract data from the mesh structure
Nx = u.domain.dims[1]
Ny = u.domain.dims[2]
G=reshape([1:(Nx+2)*(Ny+2);], Nx+2, Ny+2)
...
function convectionTermCylindrical2D{T<:Real}(u::FaceValue{T})
    # extract data from the mesh structure
    Nr = u.domain.dims[1]
    Nz = u.domain.dims[2]
    ...
    sx = zeros(T, 3*(Nr+2)*(Nz+2))
    sy = zeros(T, 3*(Nr+2)*(Nz+2))
    mnx = Nr*Nz
    mny = Nr*Nz
    # reassign the east, west for code readability
...
function convectionUpwindTermCylindrical2D{T<:Real}(u::FaceValue{T})
    # extract data from the mesh structure
    Nr = u.domain.dims[1]
    Nz = u.domain.dims[2]
    G=reshape([1:(Nr+2)*(Nz+2);], Nr+2, Nz+2)
    ...
    vn_min = min(u.yvalue[:,2:Nz+1],0)
    vn_max = max(u.yvalue[:,2:Nz+1],0)
    vs_min = min(u.yvalue[:,1:Nz],0)
    vs_max = max(u.yvalue[:,1:Nz],0)
...
function convectionTermRadial2D{T<:Real}(u::FaceValue{T})
    # extract data from the mesh structure
    Nr = u.domain.dims[1]
    ...
    sx = zeros(T, 3*(Nr+2)*(Ntheta+2))
    sy = zeros(T, 3*(Nr+2)*(Ntheta+2))
    mnx = Nr*Ntheta
    mny = Nr*Ntheta
    re = rf[2:Nr+1,:]
    rw = rf[1:Nr,:]
...
function convectionUpwindTermRadial2D{T<:Real}(u::FaceValue{T})
    # extract data from the mesh structure
    Nr = u.domain.dims[1]
    Ntheta = u.domain.dims[2]
    G=reshape([1:(Nr+2)*(Ntheta+2);], Nr+2, Ntheta+2)
    DRp = u.domain.cellsize.x[2:end-1]
    DTHETAp = Array(T, 1, Ntheta)
    ...
    # Also correct for the boundary cells (not the ghost cells)
    # Left boundary:
    APx[1,:] = APx[1,:]-rw[1,:].*uw_max[1,:]./(2*DRp[1]*rp[1,:])
    AW[1,:] = AW[1,:]/2
    # Right boundary:
    AE[end,:] = AE[end,:]/2
    APx[end,:] = APx[end,:]+re[end,:].*ue_min[end,:]./(2*DRp[end]*rp[end,:])
    # Bottom boundary:
    APy[:,1] = APy[:,1]-vs_max[:,1]./(2*DTHETAp[1]*rp[:,1])
    AS[:,1] = AS[:,1]/2
    # Top boundary:
    AN[:,end] = AN[:,end]/2
    APy[:,end] = APy[:,end]+vn_min[:,end]./(2*DTHETAp[end]*rp[:,end])
    # now reshape
    AE = reshape(AE,mnx)
    AW = reshape(AW,mnx)
    AN = reshape(AN,mny)
...
# %% =========================== 3D Convection Term ==========================
function convectionTerm3D{T<:Real}(u::FaceValue{T})
    # extract data from the mesh structure
    Nx = u.domain.dims[1]
    Ny = u.domain.dims[2]
    Nz = u.domain.dims[3]
...
function convectionUpwindTerm3D{T<:Real}(u::FaceValue{T})
    # extract data from the mesh structure
    ...
    # Also correct for the boundary cells (not the ghost cells)
    # Left boundary:
    APx[1,:,:] = APx[1,:,:]-uw_max[1,:,:]/(2*DXp[1])
    AW[1,:,:] = AW[1,:,:]/2
    # Right boundary:
    AE[end,:,:] = AE[end,:,:]/2
    APx[end,:,:] = APx[end,:,:]+ue_min[end,:,:]/(2*DXp[end])
    # Bottom boundary:
    APy[:,1,:] = APy[:,1,:]-vs_max[:,1,:]/(2*DYp[1])
    AS[:,1,:] = AS[:,1,:]/2.0
    # Top boundary:
    AN[:,end,:] = AN[:,end,:]/2
    APy[:,end,:] = APy[:,end,:]+vn_min[:,end,:]/(2*DYp[end])
    # Back boundary:
    APz[:,:,1] = APz[:,:,1]-wb_max[:,:,1]/(2*DZp[1])
    AB[:,:,1] = AB[:,:,1]/2
    # Front boundary:
    AF[:,:,end] = AF[:,:,end]/2
    APz[:,:,end] = APz[:,:,end]+wf_min[:,:,end]/(2*DZp[end])
    AE = reshape(AE,mnx)
    AW = reshape(AW,mnx)
    AN = reshape(AN,mny)
    AS = reshape(AS,mny)
...
# TODO: continue convection terms
